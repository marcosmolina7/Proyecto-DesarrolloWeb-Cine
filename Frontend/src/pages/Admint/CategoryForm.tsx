// src/pages/Admint/CategoryForm.tsx
...
const CategoryForm: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await axios.post(`${API_URL}/categorie`, { nameCategorie: name });
      setIsSuccess(true);
  ...
  return (
    <Layout>
      <div className="max-w-xl mx-auto py-8 px-4"> 
        <h1 className="text-4xl font-extrabold text-white mb-2 flex items-center"><List className="w-8 h-8 mr-3 text-blue-400" />Agregar Nueva Categoría</h1>
        <p className="text-gray-400 mb-6">Crea una nueva categoría para los productos de dulcería.</p>
          ...
          <AnimatePresence mode="wait">
            {isSuccess ? (
              <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
                ...
                <h2 className="text-2xl font-bold text-white mb-2">¡Categoría Creada!</h2>
              ...
              <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <label htmlFor="nameCategorie" className="block text-sm font-medium text-gray-400 mb-2">Nombre de la Categoría (Ej: 'Bebidas')</label>
                  <input
                    type="text"
                    id="nameCategorie"
                    value={nameCategorie}
                ...
                <div className="pt-4">
                  <button type="submit" disabled={isSaving} className="w-full flex items-center justify-center space-x-2 p-3 rounded-lg text-white bg-green-600 hover:bg-green-700 disabled:bg-gray-500">
                    ...
                    <span>{isSaving ? 'Creando...' : 'Crear Categoría'}</span>
                  </button>
                </div>
              </motion.form>
            )}
